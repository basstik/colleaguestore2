...
public class PersonFacadeBean implements PersonFacade {
	...
	private static final String URL = "http://152.66.245.130:84/restconf/operational/opendaylight-inventory:nodes/node/openflow:105951039177862/group/150010/group-statistics/buckets";

	private String getActualPacketCount(String url) {
		try {
	...
	private Long parseJsonAndReturnPacket(String result) {
		...
		// String string = ";
		Object packetCounter = JsonPath.read(document,
				"$.opendaylight-group-statistics:buckets.bucket-counter[0].packet-count");
		System.out.println("Packet: " + String.valueOf(Long.parseLong(String.valueOf(packetCounter))));
		return Long.parseLong(String.valueOf(packetCounter));
	}

	private Long parseJsonAndReturnByte(String result) {
		Object document = Configuration.defaultConfiguration().jsonProvider().parse(result);
		Object byteCounter = JsonPath.read(document,
				"$.opendaylight-group-statistics:buckets.bucket-counter[0].byte-count");
		System.out.println("Byte: " + String.valueOf(Long.parseLong(String.valueOf(byteCounter))));
		return Long.parseLong(String.valueOf(byteCounter));
	}
	...
	public MyWrapper getMap() {
		String json = getActualPacketCount(URL);
		...
		System.out.println("linkedHashMapPacket" + linkedHashMapPacket);
		System.out.println("linkedHashMapByte" + linkedHashMapByte);
